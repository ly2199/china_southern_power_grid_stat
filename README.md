...
南网的“日电费”（每日电费）功能由深圳供电局先行推出，`charge/queryDayElectricChargeByMPoint` 接口
对云南等地区的账户会返回服务端错误（`深圳电费日历查询返回:营销接口异常`）。

针对该情况，本插件已做兼容处理：

- **自动检测区域能力**：首次更新时探测账号所在区域是否支持“日电费”和“阶梯数据”，结果缓存到配置中。
  不支持的传感器（阶梯档位/阶梯剩余电量/阶梯电价、最新日电费、当月电费）将**自动删除，不再创建**；
  如云南账号仅保留有数据的传感器（余额、欠费、昨日用电量、最新日用电量、本月/上月/今年/去年电量与电费、
  本月平均温度、停电公告、停电事件）
- 当月/上月**总电费、总电量**会自动回退到月度账单接口（`charge/selectElecBillDetails`）获取，
  因此`上月电费`、`上月用电量`等传感器依然可用（当月账单在次月生成，当月电费在账单生成前不可用）
- 云南居民用电采用丰水期/枯水期分时电价，非阶梯计价，账单接口中无阶梯档位数据，故阶梯相关传感器不可用

### 传感器属性（完整信息展示）

在基础电量/电费数值之外，传感器附加了所有可获取的信息：

- **余额**：户名、用电地址、欠费状态、欠费总额、用户类型、结算户号、锁定余额、电能表资产号、
  手机号、微信昵称、实名认证状态
- **缴费记录**：状态为最近一笔缴费摘要与总笔数（如"最近缴费 2026-01-04 01:24:57 -17.29 联网自助 预收冲抵（共10条）"），
  属性含最近 10 条缴费/充值记录（时间、方式、金额、说明）
- **上月电费**：账单年月、抄表点编号、抄表日期、上期/本期表底读数、用电天数、抄表周期、计费周期、
  缴费截止日期、缴费方式、供电单位（供电所）、电价类别、结算户名、应缴电费、本期应收电费、
  违约金及日期、历史欠费、平均电价、峰/平/谷电量与电费、基本电费、需量电费、
  阶梯已用/剩余电量与使用说明（支持区域）、阶梯基数说明
- **今年累计电费**：最近 12 个月的账单历史（每月电量与电费）
- **本月平均温度**（用电日历）：当月平均/最高/最低温度、每日温度明细、日历月度汇总
  （总电量、峰/谷段统计，视区域支持）
- **本月用电日历**：状态为最新一天用电量，属性含当月**每日一条**的用电量与最高/最低/平均温度、
  月总电量、月平均/最高/最低温度
- **年度阶梯信息**：状态为完整可读说明文本（截至日期、年度累计电量、当前档位、当前电价、剩余电量、
  各档位阈值与电价、年度起止日期），属性含各项结构化字段
- **停电公告**：本地区最近一条检修/停电公告，属性含最近 12 个月的检修计划公告列表
- **停电事件**：按家庭位置（HA 设置中的经纬度，与南网APP"停电信息查询"相同逻辑）查询所在
  供电单位的计划停电与故障停电。状态为当前进行中/即将到来的停电事件数量
  （0=无停电），属性含停电范围、起止时间、事件类型、定位区域（省/市/县/镇街）
- **本月用电量/本月电费**：当月每日用电量（含每日最高/最低/平均温度；电费仅支持区域）
- **今年/去年用电量**：各月电量与电费明细
...
注意：本集成需求`Home Assistant`最低版本为`2024.6`。
